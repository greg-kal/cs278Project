...
function supabaseEventToLocal(row) {
  ...
  return {
    ...
    startsAt: row.starts_at, // Store for chronological sorting
    ...
    photoUrl: row.photo_url || null, // URL to uploaded photo
...
export function AppProvider({ children }) {
  ...
  const deleteEvent = useCallback(async (eventId) => {
    if (!supabase || !isUUID(eventId)) return { error: { message: 'Invalid event' } };
  ...
  const hideEvent = useCallback((eventId) => {
    // Hide event from local state (does not delete from database)
    setEvents(prev => prev.filter(e => e.id !== eventId));
  }, []);
  ...
  const addEvent = useCallback(async (formData, userId) => {
    ...
    const newEvent = {
      ...
      title: formData.title.trim(),
      time: displayTime,
      startsAt: d.toISOString(), // Store for chronological sorting
      dateKey,
      dateLabel: buildDateLabel(dayOffset),
      hostId: userId,
      ...
      durationNote: '',
      visibility: formData.visibility,
      photoUrl: formData.photoUrl || null, // URL to uploaded photo
      goingIds: [],
    ...
    // Insert event in chronological order instead of prepending
    setEvents(prev => {
      const newEvents = [...prev, newEvent];
      return newEvents.sort((a, b) => new Date(a.startsAt) - new Date(b.startsAt));
    });

    return { data };
  }, []);

  const updateEvent = useCallback(async (eventId, formData, userId) => {
    if (!supabase || !userId) return { error: { message: 'Not configured or not signed in' } };
    if (!formData.title.trim()) return { error: { message: 'Title is required' } };
    ...
    setEvents(prev => {
      ...
      return updated.sort((a, b) => new Date(a.startsAt) - new Date(b.startsAt));
    });
  ...
  const updateProfile = useCallback(async (updates) => {
    if (!supabase || !profile) return { error: { message: 'Not signed in' } };
  ...
  return (
    <AppContext.Provider value={{
      ...
      profile, findUser, cacheProfiles, updateProfile,
      favorites, toggleFavorite,
      following, toggleFollow,
      joined, joinEvent, leaveEvent,
      events, addEvent, updateEvent, getEventById, deleteEvent, hideEvent,
      addComment,
